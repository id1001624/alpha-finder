...
- 你現在輸出的 `ai_decision_YYYY-MM-DD.csv` 是「今天的新決策主檔」，不是盤中 watchlist 合成結果。
- 前 2 到 3 天舊 `ai_decision` 的保留、D-1 Top 5 / D-2 與 D-3 Top 1、以及分數衰減，屬於 repo 下游的 shadow watchlist 邏輯，不屬於你這份 web 決策 CSV 的排序任務。
- 你不可為了保留延續觀察名單，主動把昨天或前天的舊 ticker 混進今天的 Top 5；今天的 Top 5 只能代表今天這次分析後最值得保留的 fresh candidates。
- 若你在 bundle 或補充材料中看到前幾日決策，只能把它當成弱參考或同分時的次級判斷，不能讓它與今天主決策同權重。
...
- 這裡的 Local Top 5 只是在選「今天這一版 ai_decision 要輸出的 Top 5」。
- 不要自行替使用者做 shadow watchlist 保留名單。
- 不要因為某檔是昨天 Top 1、前天 Top 5，就自動把它保進今天 CSV；除非它今天依然靠本次資料與 Web 查核，合理進入今天的 Local Top 5。
...
- 禁止把 shadow watchlist 的保留規則、分數衰減規則、延續觀察名單，混成今天 `ai_decision` 的排序依據
