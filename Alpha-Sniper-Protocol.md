# Alpha Sniper Web Decision Prompt（v9｜Tomorrow Continuation First｜Dropbox MD Ready）
...
- 從我上傳的 `ai_ready_bundle.xlsx`，或 Dropbox `AlphaFinder/ai_ready_bundle_md/` 內的 12 個核心 md 中，找出「明天最可能延續上漲」的 Top 1
- 給我 Top 5 備選
- 告訴我為什麼是它，而不是別檔
...
1. 直接上傳 `ai_ready_bundle.xlsx`
2. 若使用 Dropbox，則直接讀取 `AlphaFinder` 應用程式下的 `ai_ready_bundle_md/` 資料夾內 12 個核心 md

若有 `ai_ready_bundle.xlsx`，它仍是單一主入口。
若沒有 xlsx，而是 Dropbox md 模式，則你必須把那 12 個 md 視為 bundle 的等價輸入，不要自行擴充到其他中間層檔案。

### B) `ai_ready_bundle.xlsx` / Dropbox md 重要閱讀順序
...
若你是從 Dropbox 讀資料，而不是直接上傳 xlsx，則只讀這 12 個 md：

- `AlphaFinder/ai_ready_bundle_md/decision_signals_daily.md`
- `AlphaFinder/ai_ready_bundle_md/ranking_signals_daily.md`
- `AlphaFinder/ai_ready_bundle_md/ai_research_candidates.md`
- `AlphaFinder/ai_ready_bundle_md/event_signals_daily.md`
- `AlphaFinder/ai_ready_bundle_md/monster_radar_daily.md`
- `AlphaFinder/ai_ready_bundle_md/xq_short_term_updated.md`
- `AlphaFinder/ai_ready_bundle_md/raw_market_daily.md`
- `AlphaFinder/ai_ready_bundle_md/theme_heat_daily.md`
- `AlphaFinder/ai_ready_bundle_md/theme_leaders_daily.md`
- `AlphaFinder/ai_ready_bundle_md/ai_focus_list.md`
- `AlphaFinder/ai_ready_bundle_md/fusion_top_daily.md`
- `AlphaFinder/ai_ready_bundle_md/api_catalyst_analysis.md`（若存在）

這 12 個 md 對應 bundle 內的核心 sheet。
不要額外要求 `market_dataset_daily`、`feature_signals_daily`、`radar_signals_daily`，除非我明確另外提供。
...
- 你無法直接寫入本機
- 若資料來自 Dropbox，優先讀取 `AlphaFinder/ai_ready_bundle_md/` 內的 12 個核心 md，視為 `ai_ready_bundle.xlsx` 的等價輸入
