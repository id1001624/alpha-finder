        ...
        rows.append(
            {
                ...
                'premarket_gap_pct': round(_safe_float(row.get('premarket_gap_pct')), 2),
                'close_location_value': round(_safe_float(row.get('close_location_value')), 4),
                'upper_wick_pct': round(_safe_float(row.get('upper_wick_pct')), 2),
                'market_data_source': row.get('market_data_source', ''),
                'market_snapshot_live': bool(row.get('market_snapshot_live', False)),
                'catalyst_verifiability_score': round(_safe_float(row.get('catalyst_verifiability_score')), 2),
                'catalyst_freshness_score': round(_safe_float(row.get('catalyst_freshness_score')), 2),
                'catalyst_strength_score': round(_safe_float(row.get('catalyst_strength_score')), 2),
                'open_exhaustion_risk_score': round(_safe_float(row.get('open_exhaustion_risk_score')), 2),
                'overnight_followthrough_score': round(_safe_float(row.get('overnight_followthrough_score')), 2),
                'monster_overlay': str(row.get('monster_overlay', 'no')).strip().lower() or 'no',
                'protocol_gate_reason': row.get('protocol_gate_reason', ''),
